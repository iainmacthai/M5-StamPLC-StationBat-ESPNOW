...
int currentRangeIndex = 1; // Start with 8G (index 1)
...
// Colors from the StamPLC example
...
// Relay display parameters (reduced size)
int relayBoxWidth = 50;
int relayBoxHeight = 30;  // Reduced height
int relayMargin = 10;
int relayStartX = 5;  // Reduced from 10 to 5 (moved 5px left)
int relayY = 135 - relayBoxHeight - 25;  // Adjusted position for footer
unsigned short relayColor[2] = {red, green}; // For the status line

// Input display parameters
int inputBoxWidth = 50;
int inputBoxHeight = 30;
int inputMargin = 10;
int inputStartX = 5;
int inputRow1Y = 40;  // First row of inputs
int inputRow2Y = 75;  // Second row of inputs
unsigned short inputColor[2] = {red, green}; // For the status line

// Battery indicator parameters
int batteryWidth = 30;
...
const char* buttonLabels[PAGE_COUNT][3] = {
  {"Page", "Refresh", "Off"},    // StamPLC Relays page
  {"Page", "Mode", "Range"},     // Magnetometer page (Button B toggles mode, Button C cycles range)
  {"Page", "", ""},              // Motion Alarms page
  {"Page", "", ""},              // StamPLC Inputs page
  {"Page", "", ""},              // Station Port A1 page
  {"Page", "", ""},              // Station Port B1 page
  {"Page", "", ""},              // Station Port C1 page
  {"Page", "", ""},              // Station Port A2 page
  {"Page", "", ""},              // Station Port B2 page
  {"Page", "", ""},              // Station Port C2 page
  {"Page", "", ""}               // Settings page
};
...
// Input state management (placeholder - will need to be updated with actual data)
...
void OnDataRecv(const esp_now_recv_info_t *info, const uint8_t *incomingData, int len) {
  ...
  if (incomingMessage.command == 0) {
    ...
    // Update display if we're on the relays page
...
void updateLEDs() {
  ...
  for (int i = 0; i < 7; i++) {
    if (i != 3) { // Skip the power LED (index 3)
  ...
  if (isCharging) {
    NeoPixel[3] = CRGB(0, 255, 0); // Green when charging
  } else {
    NeoPixel[3] = CRGB(0, 0, 255); // Blue when not charging
  }
  
  // Set the LED for the current page to blue
  switch (currentPage) {
    case PAGE_STATION_PORT_A1:
      NeoPixel[0] = CRGB(0, 0, 255); // Blue for Port A1 (RGB1)
      break;
    case PAGE_STATION_PORT_B1:
      NeoPixel[1] = CRGB(0, 0, 255); // Blue for Port B1 (RGB2)
      break;
    case PAGE_STATION_PORT_C1:
      NeoPixel[2] = CRGB(0, 0, 255); // Blue for Port C1 (RGB3)
      break;
    case PAGE_STATION_PORT_A2:
      NeoPixel[6] = CRGB(0, 0, 255); // Blue for Port A2 (RGB7)
      break;
    case PAGE_STATION_PORT_B2:
      NeoPixel[5] = CRGB(0, 0, 255); // Blue for Port B2 (RGB6)
      break;
    case PAGE_STATION_PORT_C2:
      NeoPixel[4] = CRGB(0, 0, 255); // Blue for Port C2 (RGB5)
      break;
    default:
      // For other pages, keep the default orange for port LEDs
      break;
  }
...
void drawBatteryIndicator() {
  ...
  // Determine battery color: green when charging, blue when not charging
  unsigned short batteryColor = isCharging ? green : blue;
  
  // If battery level is low (less than 20%), use red
  ...
  // Draw battery percentage text below the battery with larger font
  sprite->setTextColor(WHITE, BLACK);
  sprite->loadFont(midFont);  // Use the same font as relay labels
...
void drawTitleBar() {
  // Draw title bar background with new dimensions and color
  sprite->fillRect(0, 0, titleBarWidth, titleBarHeight, titleBarColor);
  
  // Draw page title with larger font
  sprite->setTextColor(titleTextColor, titleBarColor);
  sprite->loadFont(newFont);  // Use the larger newFont for titles
  sprite->setTextDatum(ML_DATUM);
  
  // Calculate text width to ensure it fits in the title bar
  int textWidth = sprite->textWidth(pageTitles[currentPage]);
  int maxWidth = titleBarWidth - 10;  // Leave some margin
  
  // If text is too wide, use a smaller font
  if (textWidth > maxWidth) {
    sprite->unloadFont();
    sprite->loadFont(midFont);  // Fall back to midFont if newFont is too wide
  }
  
  // Center text vertically in title bar
...
void drawFooter() {
  ...
  // Draw button labels with midFont
  sprite->setTextColor(footerTextColor, footerColor);
  sprite->loadFont(midFont);  // Changed from smallFont to midFont
  sprite->setTextDatum(MC_DATUM);
...
void drawStamPLCRelaysPage() {
  for (int i = 0; i < 4; i++) {
    int x = relayStartX + i * (relayBoxWidth + relayMargin);  // Use relayStartX instead of relayMargin
    ...
    // Draw status line - changes color based on state (moved up)
    sprite->fillRect(x + 4, relayY + 3, relayBoxWidth - 8, 5, relayColor[relayStates[i]]);
    
    // Draw relay label - always the same color (moved down)
...
void drawStamPLCInputsPage() {
  // Draw 8 input boxes in two rows (1-4 on top, 5-8 on bottom)
  for (int i = 0; i < 8; i++) {
    int row = i < 4 ? 0 : 1;
    int col = i % 4;
    ...
    // Draw input box background - always use darkblue regardless of state
    ...
    // Draw input label - always the same color
    sprite->setTextColor(0xCE59, darkblue);  // Light gray text on darkblue background
    sprite->loadFont(midFont);
    sprite->setTextDatum(MC_DATUM);
...
void drawMagnetometerPage() {
  ...
  if (qmc.getRawMagnetic(&x, &y, &z) && qmc.getGaussField(&gx, &gy, &gz)) {
    ...
    // Display mode indicator on the right side
    if (showRawValues) {
      sprite->drawString("Raw Values", 135, 75);
    } else {
      sprite->drawString("Gauss Values", 135, 75);
    }
    
    // Display current range on the right side below the mode
...
void drawMotionAlarmsPage() {
  // Placeholder for motion alarms page
  sprite->setTextColor(WHITE, BLACK);
  sprite->loadFont(midFont);
  sprite->setTextDatum(MC_DATUM);
...
void drawStationPortA1Page() {
  // Placeholder for Station Port A1 page
  sprite->setTextColor(WHITE, BLACK);
  sprite->loadFont(midFont);
  sprite->setTextDatum(MC_DATUM);
...
void drawStationPortB1Page() {
  // Placeholder for Station Port B1 page
  sprite->setTextColor(WHITE, BLACK);
  sprite->loadFont(midFont);
  sprite->setTextDatum(MC_DATUM);
  sprite->drawString("Port B1 Data", 120, 70);
  sprite->unloadFont();
}

void drawStationPortC1Page() {
  // Placeholder for Station Port C1 page
  sprite->setTextColor(WHITE, BLACK);
  sprite->loadFont(midFont);
  sprite->setTextDatum(MC_DATUM);
  sprite->drawString("Port C1 Data", 120, 70);
  sprite->unloadFont();
}

void drawStationPortA2Page() {
  // Placeholder for Station Port A2 page
  sprite->setTextColor(WHITE, BLACK);
  sprite->loadFont(midFont);
  sprite->setTextDatum(MC_DATUM);
  sprite->drawString("Port A2 Data", 120, 70);
  sprite->unloadFont();
}

void drawStationPortB2Page() {
  // Placeholder for Station Port B2 page
  sprite->setTextColor(WHITE, BLACK);
  sprite->loadFont(midFont);
  sprite->setTextDatum(MC_DATUM);
  sprite->drawString("Port B2 Data", 120, 70);
  sprite->unloadFont();
}

void drawStationPortC2Page() {
  // Placeholder for Station Port C2 page
  sprite->setTextColor(WHITE, BLACK);
  sprite->loadFont(midFont);
  sprite->setTextDatum(MC_DATUM);
  sprite->drawString("Port C2 Data", 120, 70);
  sprite->unloadFont();
}
...
void drawSettingsPage() {
  ...
  sprite->loadFont(midFont);
  sprite->setTextDatum(MC_DATUM);
  sprite->drawString("Settings", 120, 70);
  sprite->unloadFont();
}

void updateDisplay() {
  // Update the relayStates array based on relayState
  for (int i = 0; i < 4; i++) {
...
void drawCurrentPage() {
  ...
  // Draw title bar and battery indicator on all pages
  drawTitleBar();
  drawBatteryIndicator();
  
  // Draw page-specific content
  switch (currentPage) {
    case PAGE_STAMPLC_RELAYS:
      drawStamPLCRelaysPage();
      break;
    case PAGE_MAGNETOMETER:
      drawMagnetometerPage();
      break;
    case PAGE_MOTION_ALARMS:
      drawMotionAlarmsPage();
      break;
    case PAGE_STAMPLC_INPUTS:
      drawStamPLCInputsPage();
      break;
    case PAGE_STATION_PORT_A1:
      drawStationPortA1Page();
      break;
    case PAGE_STATION_PORT_B1:
      drawStationPortB1Page();
      break;
    case PAGE_STATION_PORT_C1:
      drawStationPortC1Page();
      break;
    case PAGE_STATION_PORT_A2:
      drawStationPortA2Page();
      break;
    case PAGE_STATION_PORT_B2:
      drawStationPortB2Page();
      break;
    case PAGE_STATION_PORT_C2:
      drawStationPortC2Page();
      break;
    case PAGE_SETTINGS:
      drawSettingsPage();
      break;
...
if (qmc.begin()) {
  ...
  // Initialize display with proper settings for StationBat
  M5.Display.init();
  M5.Display.setRotation(1); // May need to adjust rotation if display is upside down
  M5.Display.setBrightness(255);
  
  // Create sprite for display (240x135 resolution)
...
void loop() {
  ...
  // Update magnetometer display if on the magnetometer page
  ...
  // Button B: Toggle between raw and Gauss values on magnetometer page
  ...
  // Button C: Cycle through magnetometer ranges or toggle all relays off
  if (M5.BtnC.wasPressed()) {
    if (currentPage == PAGE_MAGNETOMETER) {
      ...
      // Update display to show new range
      updateDisplay();
